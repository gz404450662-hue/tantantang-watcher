...
const queryActivity = (searchKey: string) => {
    const reqData = {
        "page": "1",
        "count": "10",
        "title": searchKey,
...
const getActivityDetail = (activityId: string) => {
    ...
    return client.post("/api/shop/activity_detail", reqData)
}

queryActivity('蛙小侠(五月花店)🔥🔥经典干蛙锅双人餐').then(res => console.log(res.data.data))